  ...
  useEffect(() => {
    const fetchVendor = async () => {
      try {
        ...
        const mockVendor = {
          _id: params.vendorId,
          name: params.vendorId === '1' ? 'Homely Tiffins' : 
                params.vendorId === '2' ? 'Spice Garden' : 
                params.vendorId === '3' ? 'Mom\'s Kitchen' : 'Sample Vendor',
    ...
    <div className="container mx-auto px-4 py-12 max-w-6xl">
      <div className="text-center mb-12">
        <h1 className="text-4xl font-bold mb-4 bg-gradient-to-r from-primary to-secondary bg-clip-text text-transparent">
          {vendor.name} Subscription Plans
        </h1>
        <p className="text-xl text-gray-600 max-w-3xl mx-auto">
          Subscribe to regular tiffin deliveries from {vendor.name} and enjoy delicious, home-cooked meals every day.
        </p>
      </div>

      {/* Coming Soon Banner */}
      <div className="bg-gradient-to-r from-primary to-secondary text-white rounded-lg shadow-lg p-8 mb-12 text-center">
        <h2 className="text-3xl font-bold mb-4">Vendor-Specific Subscription Coming Soon!</h2>
        <p className="text-xl mb-6">
          We&apos;re working hard to bring you convenient subscription plans for regular tiffin deliveries from {vendor.name}.
        ...
        {!submitted ? (
          ...
          <div className="bg-white/20 rounded-lg p-4 max-w-md mx-auto">
            <p className="text-white font-medium">
              Thank you! We&apos;ll notify you when {vendor.name} subscriptions are available.
            </p>
          </div>
        )}
      </div>
      ...
      <div className="bg-white rounded-lg shadow-lg p-8">
        ...
        <div className="space-y-6 max-w-3xl mx-auto">
          <div>
            <h3 className="text-xl font-semibold mb-2">How will the {vendor.name} subscription work?</h3>
            <p className="text-gray-700">
              Our subscription service will allow you to pre-pay for daily tiffin deliveries from {vendor.name}. 
              You&apos;ll be able to customize your meals daily and enjoy significant savings compared to ordering individually.
            </p>
          </div>
          ...
          <div>
            <h3 className="text-xl font-semibold mb-2">When will {vendor.name} subscriptions be available?</h3>
            <p className="text-gray-700">
      ...
      <div className="mt-8 text-center">
        <Link href={`/vendor/${vendor._id}`} className="btn-secondary inline-block mr-4">
          Back to {vendor.name}
        </Link>
